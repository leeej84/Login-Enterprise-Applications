// IE Multi-Tab Script version 1.0
// Open a random number of tabs in IE and select a website from a list at random to open
...
public class IE_MultiTab : ScriptBase
{
    ...
    {           
        ...
        while( x != randomTabs ) {       
            //Open a new tab
            MainWindow.FindControl(className : "Button", title : "New tab (Ctrl+T)").Click();
            
            //Select the address bar
            MainWindow.FindControl(className : "Pane:Address Band Root", title : "Address Bar").Click();
            ...
            //Allow local content
            //Currently an issue - raised a ticket with LVSI support
            try {
                MainWindow.FindControl(className : "Button", title : "Allow blocked content", timeout: 10 ,continueOnError: true).Click();
            } catch {
                Log("IE blocked message did not appear, skipping");
            }
